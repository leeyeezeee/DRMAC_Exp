...
class DRMACAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state, encoder_hidden_state, weight=None):
    ...
    def vae_withnoise_forward(self, inputs, encoder_hidden_state):
        bs = inputs.shape[0] // self.args.n_agents
        if self.args.noise_env:
            noise = th.randn(bs*self.args.n_agents, inputs.shape[-1])
            inputs += noise.to(inputs.device)
        recons, input, z, encoder_hidden_state = self.encoder(inputs, encoder_hidden_state)
        noisy_encoder = copy.deepcopy(self.encoder)
        self._add_gaussian_noise_to_encoder(noisy_encoder)
        _, _, z_noisy, _ = noisy_encoder(inputs, encoder_hidden_state)
        return recons, input, z, z_noisy, encoder_hidden_state
    ...
    def _add_gaussian_noise_to_encoder(self, encoder):
        eta = 1.0  
        for param in encoder.parameters():
            noise = th.normal(0, eta * th.ones_like(param.data) * param.data.std())
            param.data.add_(noise)
    
    def rl_forward(self, inputs, state_repr, hidden_state, weight=None):
        # inputs.shape: [batch_size*n_agents, input_shape]
        bs = inputs.shape[0] // self.args.n_agents
