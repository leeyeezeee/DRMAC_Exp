...
class ObAttnAEEnc(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs) -> None:
        super(ObAttnAEEnc, self).__init__()
        ...
        # decorelation config
        # Determine the input dimension for attention layers
        if self.args.use_decorelation:
            self.decorelation = BarlowTwins(args)
            # attn_input_dim = self.decorelation.projector_last_size  # BarlowTwins 的 projector 最后输出维度
        # else:
        attn_input_dim = input_shape
        ...
        self.query = nn.Linear(attn_input_dim, self.args.attn_embed_dim)
        self.key = nn.Linear(attn_input_dim, self.args.attn_embed_dim)
        self.value = nn.Linear(attn_input_dim, self.args.encoder_hidden_dim)
        ...
        modules = []
        last_h_dim = self.args.encoder_hidden_dim
    ...
    def encode(self, inputs, encoder_hidden_state):
        ...
        # if self.args.use_decorelation:
        #     inputs = self.decorelation(inputs)
    ...
    def decorelation_forward(self, inputs, z, encoder):
        return self.decorelation(inputs, z, encoder)
