...
class DRMACMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, teacher_forcing=False,meta_model=None):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        agent_outputs = self.forward(ep_batch, t_ep, test_mode=test_mode,meta_model=meta_model)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        return chosen_actions

    def forward(self, ep_batch, t, test_mode=False, meta_model=None):
        agent_inputs = self._build_inputs(ep_batch, t)
        avail_actions = ep_batch["avail_actions"][:, t]
        if self.args.use_metamodel:
            weight = meta_model(agent_inputs)
            agent_outs, self.hidden_states, self.encoder_hidden_states = self.agent(agent_inputs, self.hidden_states, self.encoder_hidden_states, weight)
        else:
    ...
    def rl_forward(self, ep_batch, state_repr, t, test_mode=False, meta_model=None):
        # Go through downstream rl agent
        agent_inputs = self._build_inputs(ep_batch, t)
        if self.args.use_metamodel:
            weight = meta_model(agent_inputs)
            agent_outs, self.hidden_states = self.agent.rl_forward(agent_inputs, state_repr, self.hidden_states,weight)
        else:
    ...
    def vae_withnoise_forward(self, ep_batch, t, test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)
        if "vae" in self.args.state_encoder:
            ...
            recons, input, z, z_noisy, self.encoder_hidden_states = self.agent.vae_withnoise_forward(agent_inputs, self.encoder_hidden_states)
            return recons, input, z, z_noisy  # 
        else:
    ...
    # def decorelation_forward(self, ep_batch, t, z, test_mode=False):
    #     agent_inputs = self._build_inputs(ep_batch, t)
    #     loss = self.agent.decorelation_forward(agent_inputs, self.encoder_hidden_states, z)
    #     return loss
